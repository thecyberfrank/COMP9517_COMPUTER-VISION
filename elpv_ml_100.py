...
Original file is located at
    https://colab.research.google.com/drive/120iyoCYvuqhyEAZeF96lNSptvDsyunNA
...
def load_dataset(fname='labels.csv',GaussianBlur=False, image_resize=False, new_size=(64, 64), enhance_contrast=False, equalize_hist=False, augment=False):
    ...
    # If data augmentation was performed, adjust probability and label types accordingl
    if augment:
...
# Use stratify for stratified sampling
...
# Calculate average accuracy and F1 scor
...
# Number of samples in poly and mono test sets
n_test_poly = len(X_test_poly)
n_test_mono = len(X_test_mono)
total_samples = n_test_poly + n_test_mono
...
# Print overall RF+SVM accuracy and F1 score
print(f"RF+SVM - Overall Accuracy: {total_accuracy_svm:.5f}, Overall F1 Score: {total_f1_score_svm:.5f}")
print(f"Poly RF+SVM - Avg Accuracy: {average_accuracy_poly_svm}, Best Accuracy: {best_accuracy_poly_svm}, Avg F1 Score: {average_f1_score_poly_svm}, Best F1 Score: {best_f1_score_poly_svm}")
print(f"Mono RF+SVM - Avg Accuracy: {average_accuracy_mono_svm}, Best Accuracy: {best_accuracy_mono_svm}, Avg F1 Score: {average_f1_score_mono_svm}, Best F1 Score: {best_f1_score_mono_svm}")

# Display the confusion matrix and classification report for the last iteration
plot_confusion_matrix(last_svm_cm_poly, classes=list(label_mapping.values()), title='Poly RF+SVM Confusion Matrix')
print("Poly RF+SVM Classification Report:\n", last_svm_report_poly)
plot_confusion_matrix(last_svm_cm_mono, classes=list(label_mapping.values()), title='Mono RF+SVM Confusion Matrix')
print("Mono RF+SVM Classification Report:\n", last_svm_report_mono)